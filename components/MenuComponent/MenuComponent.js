...
const MenuComponent = ({ className, userToken, setUserToken, strings: { title, options, lists }}) => {

    useEffect(() => {
        if(window && !userToken && window.location.hash.split("&")[0] !== undefined){
    ...
    useEffect(() => {
        axios('https://accounts.spotify.com/api/token', {
            ...
            headers: {
                ...
                'Authorization': 'Basic ' + btoa('445566ef901e4c2fbdcc0c23906d027f' + ':' + '8af7ecc0f2fc44b79ea03414ef222390')
            },
    ...
    const elements = (opt) => {
        return opt.map((item, key) => {
            return(
                <ul className="menu-component__container__list__element" onClick={()=>goTo(item.url)} key={key}>
                    <span className="menu-component__container__list__element__text">{item.title}</span>
                </ul>
            )
        }
    )};
    ...
    return (
        <div className={`${className} menu-component`}>
            {checkLoging()}
        </div>
    );
};
